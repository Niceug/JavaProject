...
///////////////////////////////////////////////////////////////////////////
		...
		JButton countDepartmentBtn = new JButton("各院系申报及审核情况");
		countDepartmentBtn.setBounds(20, 700, 150, 50);
...
///////////////////////////////////////////////////////////////////////////
		...
		CheckedBtn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				List<Project> list = ProjectDao.queryAllCheckedDB();
				updateTable(list, cstr, str, jp);
			}
		});
		...
		countBtn.addMouseListener( new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				new StatisticsFrame("统计项目总数", "全校申报项目总数：");
			};
		}
		);

		// 统计学院项目总数
		countDepartmentBtn.addMouseListener( new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				new StatisticsDepartmentFrame();
			};
		}
		);
	...
	public void updateTable(List<Project> list, String [][] cstr, String[] str, JPanel jp) {
		...
		if(list != null) {
			...
			for (Project project : list) {
				...
				cstr[j][4] = project.getDepartment();
				cstr[j][5] = project.getProType();
				cstr[j][6] = project.getProForm();
				cstr[j][7] = project.getCreateTime().toString();
				cstr[j][8] = project.getContent();
				cstr[j][9] = project.getMemo();
				cstr[j][10] = project.getChecked();
